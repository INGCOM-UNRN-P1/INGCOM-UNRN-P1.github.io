...
:::{exercise} Encender un Flag Dado un `unsigned char` que representa un
...
:::{exercise} Intercambio con XOR Escribí una función que intercambie los
...
:::{exercise} Crear una Máscara para Apagar Escribí una expresión que cree una
máscara para apagar el 2do bit (posición 1) de un `char`, sin afectar a los
demás. ::: :::{solution} :class: dropdown
...
:::{exercise} Multiplicación y División Rápida Escribí dos macros,
...
:::{exercise} Escribí una función `int get_bit(int numero, int n)` que devuelva
...
:::{exercise} Escribí una función `void set_bit(int *numero, int n)` que
...
:::{exercise} Escribí una función `void clear_bit(int *numero, int n)` que
...
:::{exercise} Implementar una función que cuente el número de bits que están en
`1` en la representación binaria de un número, usando el algoritmo de Brian
Kernighan. :::

:::{solution} :class: dropdown **Algoritmo de Brian Kernighan**: La operación
...
:::{exercise} Escribí una función que determine si un número entero positivo es
una potencia de dos (2, 4, 8, 16...). :::

:::{solution} :class: dropdown **Propiedad**: Un número que es potencia de dos
