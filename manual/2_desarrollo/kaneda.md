...
**Rol en el ecosistema:** Auditoría de vulnerabilidades en C: prohibición de
`gets()`, `strcpy()`, `sprintf()`, detección de desbordes de buffer estáticos y
...
La herramienta **`kaneda`** forma parte del ecosistema oficial de software de la
...
La interfaz CLI de `kaneda` cuenta con autocompletado nativo para comandos,
...
Para incorporar `kaneda` de forma automática a tu flujo de desarrollo, agregá la
...
Ejecutá `make check-kaneda` antes de cada commit para mantener que tu código
...
- **Tecnología Núcleo:** `Clang Static Analyzer + Semgrep C Rules + Dangerous
  Functions AST Matcher`.
...
Ninguna herramienta opera de forma aislada. **`kaneda`** forma parte del
...
- `dredd (bloqueo de entregas vulnerables)` | Informes Markdown, diagnósticos
  Rich, JSON, actas |
| **Sincronización** | `ripley`, `daedalus`, `dredd` | Validación cruzada, flags compartidos y autofix |

### Pipeline de Integración Recomendado

Podés encadenar `kaneda` con otras herramientas del ecosistema en una única
línea de comando:
...
Para crear tus propias reglas, conectores de evaluación o integrar `kaneda`
