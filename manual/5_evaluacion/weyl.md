...
subtitle: "Weyl — Diffing Semántico y Comparación Estructural de ASTs contra resolvé Canónicas"
...
:class: note
**Rol en el ecosistema:** Comparación estructural y semántica de código C entre
la entrega del estudiante y la solución de referencia docente, ignorando cambios
cosméticos de nombres de variables, espacios y comentarios.
...
La herramienta **`weyl`** forma parte del ecosistema oficial de software de la
...
La interfaz CLI de `weyl` cuenta con autocompletado nativo para comandos, flags
...
Verificar si el algoritmo implementado coincide estructuralmente con la solución
modelo.
...
Para incorporar `weyl` de forma automática a tu flujo de desarrollo, agregá la
...
Ejecutá `make check-weyl` antes de cada commit para mantener que tu código
...
- **Tecnología Núcleo:** `Tree-Sitter C AST Structural Normalizer + Levenshtein
  AST Distance Calculator + Canonical Diff Engine`.
...
Ninguna herramienta opera de forma aislada. **`weyl`** forma parte del pipeline
...
Podés encadenar `weyl` con otras herramientas del ecosistema en una única línea
de comando:
...
Para crear tus propias reglas, conectores de evaluación o integrar `weyl`
