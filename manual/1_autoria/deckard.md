...
**Rol en el ecosistema:** Curaduría pedagógica de ejercicios, composición de
guías según carga horaria y niveles Bloom, exportación a Typst/Jupyter, starter
ZIPs y control LanguageTool.
...
La herramienta **`deckard`** forma parte del ecosistema oficial de software de
...
La interfaz CLI de `deckard` cuenta con autocompletado nativo para comandos,
...
Verificar que los ejercicios no contengan términos vagos ni falta de casos
borde.
...
Para incorporar `deckard` de forma automática a tu flujo de desarrollo, agregá
...
Ejecutá `make check-deckard` antes de cada commit para mantener que tu código
...
- **Tecnología Núcleo:** `Typst Compiler + PyYAML + Bloom Taxonomy Analyzer +
  ROT13 Hints Obfuscator + Zipfile Engine`.
...
Ninguna herramienta opera de forma aislada. **`deckard`** forma parte del
...
- `alucarD (exámenes)`
- `Estudiantes (starter ZIPs)` | Informes Markdown, diagnósticos Rich, JSON,
  actas |
| **Sincronización** | `dredd`, `alucarD`, `idkfa`, `myst-tools` | Validación cruzada, flags compartidos y autofix |

### Pipeline de Integración Recomendado

Podés encadenar `deckard` con otras herramientas del ecosistema en una única
línea de comando:
...
Para crear tus propias reglas, conectores de evaluación o integrar `deckard`
