...
:class: note
**Rol en el ecosistema:** Traducción de informes crípticos generados por
AddressSanitizer (ASan) y UndefinedBehaviorSanitizer (UBSan) a explicaciones
claras en español rioplatense, con diagramas de memoria y sugerencias directas.
...
La herramienta **`tetsuo`** forma parte del ecosistema oficial de software de la
...
La interfaz CLI de `tetsuo` cuenta con autocompletado nativo para comandos,
...
🔧 SOLUCIÓN:
  verificá que tus lazos y accesos cumplan '0 <= indice < 4'.
...
Ejecutar un código que accede a memoria liberada y analizar la traducción de
Tetsuo.
...
Para incorporar `tetsuo` de forma automática a tu flujo de desarrollo, agregá la
...
Ejecutá `make check-tetsuo` antes de cada commit para mantener que tu código
...
- **Tecnología Núcleo:** `LLVM AddressSanitizer / UndefinedBehaviorSanitizer
  Parser + Shadow Memory Mapper + Spanish Pedagogical Translator`.
...
Ninguna herramienta opera de forma aislada. **`tetsuo`** forma parte del
...
- `dredd (informe en alumno_rN.md)` | Informes Markdown, diagnósticos Rich,
  JSON, actas |
| **Sincronización** | `daedalus`, `hal`, `nostromo`, `dredd` | Validación cruzada, flags compartidos y autofix |

### Pipeline de Integración Recomendado

Podés encadenar `tetsuo` con otras herramientas del ecosistema en una única
línea de comando:
...
Para crear tus propias reglas, conectores de evaluación o integrar `tetsuo`
