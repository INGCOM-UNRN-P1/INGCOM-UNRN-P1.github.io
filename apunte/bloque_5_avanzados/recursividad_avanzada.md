...
```{figure} ../bloque_1_fundamentos/8/divide_conquista_merge.svg
...
```{figure} ../bloque_1_fundamentos/8/traza_merge_sort.svg
...
Una implementación ingenua de Merge Sort reserva memoria dinámica mediante `malloc` dentro de la función `merge` en cada nivel de recursión para crear subarreglos temporales. 
...
En C, toda asignación dinámica mediante `malloc` o `calloc` puede fallar si el sistema no dispone de suficiente memoria. Para asegurar la robustez del programa, es obligatorio verificar que los punteros obtenidos no sean `NULL` antes de utilizarlos (regla {ref}`0x3001h`).
:::

:::{note} Complejidad
La relación de recurrencia para Merge Sort es $T(n) = 2T(n/2) + O(n)$. Esto se resuelve a una complejidad temporal de $O(n \log n)$. Para un análisis matemático de cómo se resuelve esta recurrencia, consultá la sección de Complejidad.
:::
...
Escribí una función recursiva en C que encuentre el elemento máximo de un arreglo de enteros utilizando el paradigma de Divide y Vencerás. La firma debe ser `int maximo_divide_y_venceras(const int arr[], size_t ini, size_t fin)`.
:::
...
La profundidad máxima del *call stack* es $O(\log n)$ debido a la división binaria del espacio.
:::
