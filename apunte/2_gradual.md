...
int main() {
    ...
    for (int i = 0; i < tamano; i++) {
        if (numeros[i] == numeroBuscado) {
            ...
            break; // Podemos salir del lazo una vez que lo encontramos
        }
    }
...
### Variable de Control de lazo (o Iterador)

Este es un rol que a menudo usamos sin siquiera pensarlo. La **variable de
control de lazo** es la que se encarga de controlar las repeticiones de un
...
int main() {
    // 'i' es la variable de control de lazo
    for (int i = 0; i < 5; i++) {
...
En este caso, `i` controla cuántas veces se ejecuta el lazo `for` y en qué
