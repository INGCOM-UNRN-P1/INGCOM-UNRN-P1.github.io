...
int main(void)
{
    ...
    direcciones[contador++] =
        (info_direccion_t){.direccion = (void *)main,
    ...
    direcciones[contador++] = (info_direccion_t){
        .direccion = (void *)&global, .descripcion = "global (Segmento Data)"};

    // 4. Variable estática global (Segmento Data)
    direcciones[contador++] =
        (info_direccion_t){.direccion = (void *)&estatica,
    ...
    if (dinamica != NULL)
    {
        *dinamica = 76;
        // 7. Segmento Heap
        direcciones[contador++] =
            (info_direccion_t){.direccion = (void *)dinamica,
                               .descripcion = "dinamica (Segmento Heap)"};
    }

    // 8. Segmento Stack
    direcciones[contador++] = (info_direccion_t){
        .direccion = (void *)&local, .descripcion = "local (Segmento Stack)"};
    ...
    {
        // Ordenación
        qsort(direcciones, contador, sizeof(info_direccion_t),
              compara_direcciones);

        printf("Direcciones de memoria de todos los segmentos ordenadas:\n");
        printf("--------------------------------------------------------\n");

        // Impresión de resultados (invertidos, así se parece mas al diagrama
        // de la presentación)
