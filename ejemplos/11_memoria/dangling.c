...
int main()
{
    ...
    *p_num = 42;
    ...
    printf("         Todavía apunta a: %p, pero esta memoria ya no es "
           "nuestra.\n\n",
           (void *)p_num);
    ...
    // basura, o podría parecer que funciona (lo que es peor, porque el error
    // está oculto).
    printf("Paso 5: Intentando acceder al puntero colgante (comportamiento "
           "indefinido)...\n");
    // La siguiente línea es peligrosa y no debe hacerse en código real.
    printf("         El valor en la memoria liberada podría ser: %d (¡valor "
           "basura o crash!)\n\n",
           *p_num);

    // --- SOLUCIÓN ---
    // 6. La buena práctica es establecer el puntero a NULL después de
    // liberarlo. Esto previene que se use accidentalmente.
    p_num = NULL;
