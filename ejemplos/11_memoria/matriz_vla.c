...
int main()
{
     ...
     *
     * 3. `malloc(...)`: Solicita un bloque contiguo de memoria de ese tamaño
     * total.
     *
     * 4. `(int (*)[columnas])`: Este es el cast (conversión de tipo). Le dice
     * al compilador que trate el puntero genérico `void*` que devuelve
     * `malloc` como un "puntero a un array de `columnas` enteros". Esto es
     * crucial para que el compilador sepa cómo calcular los desplazamientos de
     * memoria al acceder a elementos como `m[i][j]`.
     */
    int (*m)[columnas] =
        (int (*)[columnas])malloc(sizeof(int) * columnas * filas);
...
void imprimir_matriz(int filas, int columnas, int matriz[][columnas])
{
    ...
    for (int i = 0; i < filas; i++)
    {
        for (int j = 0; j < columnas; j++)
        {
            // %-5d alinea los números a la izquierda con un ancho de 5
            // caracteres
