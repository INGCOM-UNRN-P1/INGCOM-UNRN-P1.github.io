...
int main()
{
    ...
    // 3. ¡El ataque! Escribimos en `buffer` una cadena mucho más grande que su
    // capacidad. La cadena tiene más de 16 caracteres, por lo que se
    // "desbordará" y comenzará a sobrescribir la memoria que le sigue, que es
    // donde se encuentra `secret_data`.
    ...
    printf("Como se puede ver, al escribir más allá de los límites del "
           "'buffer',\n");
    printf("hemos 'solapado' y sobrescrito la memoria asignada para "
           "'secret_data'.\n");

    // 5. Liberamos la memoria (aunque el heap ya podría estar en un estado
    // inestable).
    free(buffer);
    free(secret_data);
