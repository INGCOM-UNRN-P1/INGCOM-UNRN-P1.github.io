...
int main()
{
    ...
    printf("Creando una matriz simulada de %d filas y %d columnas con un "
           "puntero simple.\n\n",
           filas, columnas);
     ...
     * 2. Se calcula el número total de elementos necesarios: `filas *
     * columnas`.
...
/**
 * @brief Rellena la matriz con valores de ejemplo usando aritmética de
 * punteros.
 * @param filas El número de filas de la matriz.
 * @param columnas El número de columnas de la matriz.
 * @param matriz Un puntero al primer elemento del bloque de memoria de la
 * matriz.
 */
void llenar_matriz(int filas, int columnas, int *matriz)
{
    for (int i = 0; i < filas; i++)
    {
        ...
        {
            /*
             * Esta es la fórmula clave para simular un acceso 2D en un array
             * 1D. (i * columnas) calcula el desplazamiento para llegar al
             * inicio de la fila `i`.
             * `+ j` calcula el desplazamiento adicional para llegar a la
             * columna `j` dentro de esa fila.
             */
...
void imprimir_matriz(int filas, int columnas, int *matriz)
{
    ...
    for (int i = 0; i < filas; i++)
    {
        for (int j = 0; j < columnas; j++)
        {
            // Se utiliza la misma fórmula para acceder e imprimir cada
            // elemento.
