...
int main()
{
    ...
    printf("Realizando una serie de asignaciones de memoria de diferentes "
           "tamaños...\n");
    ...
    for (int i = 0; i < NUM_ALLOCATIONS; i++)
    {
        ...
        printf("Puntero %d asignado con %d bytes en la dirección: %p\n", i,
               size_to_allocate, pointers[i]);
    }

    printf("\nLiberando los punteros de índice par para crear huecos en la "
           "memoria...\n");
    ...
    if (large_pointer == NULL)
    {
        ...
        printf(
            "Esto probablemente se deba a la fragmentación de la memoria.\n");
        printf("Aunque puede haber suficiente memoria libre en total, no hay "
               "un bloque contiguo lo suficientemente grande.\n");
    }
    else
    {
        printf("\nResultado: La asignación de memoria grande fue exitosa en "
               "la dirección: %p\n",
               large_pointer);
        printf("Esto puede suceder si el sistema operativo pudo encontrar un "
               "bloque contiguo o si el 'heap' no estaba lo suficientemente "
               "fragmentado.\n");
        free(large_pointer);
    }
