...
int main()
{
    ...
    printf("Buscando el tamaño máximo de asignación de memoria con un límite "
           "de ~%s...\n",
           high_formatted);
    printf("Esto puede tardar unos segundos.\n\n");

    // Usamos una búsqueda binaria para encontrar el tamaño máximo de forma
    // eficiente.
    while (low <= high)
    {
        ...
        if (mem != NULL)
        {
            ...
            // Ahora intentamos buscar un bloque aún más grande en la mitad
            // superior.
            low = mid + 1;
        }
        else
    ...
    if (max_alloc_possible > 0)
    {
        ...
        format_bytes(max_alloc_possible, formatted_size,
                     sizeof(formatted_size));
        ...
        printf("========================================================\n\n");
        printf(
            "Nota: El valor es menor al límite porque el propio programa\n");
        printf("   (código, pila, librerías, etc.) ya consumen una parte de "
               "la memoria.\n");
    }
    else
    {
        printf("\nNo se pudo asignar ninguna cantidad significativa de "
               "memoria.\n");
        printf("Asegúrate de haber establecido el límite con 'ulimit' "
               "correctamente.\n");
    }
