...
typedef enum
{
    ...
    // Binario: 1000. Una constante adicional para un permiso compuesto si se
    // necesita.
...
int main()
{
    ...
    // Utilizamos un entero (generalmente 'int' o 'unsigned int') para
    // almacenar la máscara.

    // 2. Asignación de permisos
    // Se utiliza el operador OR a nivel de bit (|) para COMBINAR múltiples
    // permisos. Combinacion binaria: 0001 | 0010 = 0011 (Valor decimal: 3)
    ...
    verificar_permiso(permisos_usuario,
                      PERMISO_EJECUCION); // Debería ser FALLO
    ...
    verificar_permiso(permisos_usuario,
                      PERMISO_EJECUCION); // Ahora debería ser OK

    // 6. Revocar un permiso (Desactivar un bit)
    // Se utiliza AND a nivel de bit (&) con la negación a nivel de bit (~) del
    // permiso.
    ...
    verificar_permiso(permisos_usuario,
                      PERMISO_LECTURA); // Ahora debería ser FALLO
