---
title: "Regla 0x1012h: Prohibición de expresiones de asignación dentro de estructuras de control"
short_title: "0x1012h"
...
:class: note
Código de regla: `0x1012h` | Soporta corrección automática: **No**
:::
...
uso; la restricción es pedagógica. Se complementa con {ref}`0x1009h`, que
...
- **`strlen` en la condición del `for`:** repetir `strlen(s)` cada iteración es
  O(n²); guardar la longitud antes del lazo (ver **{ref}`0x301Bh`**).
...
- **`for` con dos asignaciones:** el encabezado acepta varias, pero si hay
  efectos secundarios conviene un `while` (ver {ref}`0x100Dh`).
...
* {ref}`0x1009h` — prohíbe la asignación simple usada como condición.
* {ref}`0x3003h` — no mezclar asignación y comparación en una línea.
* {ref}`0x100Dh` — el `for` con corte compuesto corresponde a un `while`.
...
* {ref}`0x1012h` — prohíbe asignaciones embebidas en condiciones.
