...
* **[AP-0x2011h: Recursión mutua o cíclica sin caso base](ap_0x2011h.md)** — funciones que se llaman sin condición de corte.
:::
...
{ref}`0x3007h` y {ref}`0x0003h`. Si el parámetro debe modificarse, se pasa un
puntero.
...
local. La recursión sin caso base que describe {ref}`ap_0x2011h` suele venir
...
desborde de pila que documenta {ref}`ap_0x2011h`.
...
* {ref}`0x0003h` — toda variable mutable debe nacer con un valor conocido.
* {ref}`0x200Dh` — el resultado se concentra en una local, no en el parámetro.
