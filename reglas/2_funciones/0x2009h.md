...
* **[AP-0x2009h: Llamada recursiva sin caso base explícito](ap_0x2009h.md)** — función recursiva mal construida que agota la pila.
:::
...
por bloques. Dividir permite probar cada pieza con `assert` ({ref}`0x2006h`),
nombra su intención ({ref}`0x0001h`) y deja `main` como un índice legible.
...
({ref}`ap_0x2009h`). Una función propia debe tener su contrato completo.
...
* {ref}`0x2006h` — con funciones, cada caso se prueba con un `assert`.
