---
title: "Regla 0x200Eh: Comentarios de cierre explicativos en bloques de control extensos (> 25 líneas)"
short_title: "0x200Eh"
...
:class: note
Código de regla: `0x200Eh` | Soporta corrección automática: **No**
:::
...
complemento del umbral cuantitativo de {ref}`0x6003h`.
...
cátedra adopta el umbral de 25 líneas, el mismo de {ref}`0x6003h`, para que la
señal sea consistente. El comentario debe explicar el porqué o la estructura
({ref}`0x000Ah`), nunca ser un `// fin` vacío.
...
es ruido y puede llegar a violar {ref}`0x000Fh` (comentarios obvios).
...
es especialmente útil cuando el cuerpo supera el umbral de {ref}`0x6003h`.

### ⚠️ Casos límite

- **Umbral:** "más de 25 líneas" se cuenta sobre el cuerpo efectivo, sin
  comentarios ni líneas en blanco, igual que {ref}`0x6003h`.
...
* {ref}`0x6003h` — comparte el umbral de 25 líneas con esta regla.
* {ref}`0x000Fh` — un `// fin` vacío es un comentario redundante.
