...
Por qué falla: `atoi("hola")` devuelve `0`, y `atoi("999999999999")` desborda
con
...
- **`strtol` con `errno`:** hay que ponerlo en `0` **antes** de llamar; si no,
  se
...
* {ref}`0x500Bh` — incluí `<stdlib.h>`, `<errno.h>` y `<limits.h>` para validar
  las firmas.
* {ref}`0x5002h` — `-Wall -Wextra -Werror` pone en evidencia las advertencias
  asociadas.
...
- [ ] ¿Recorté el `'\n'` que deja `fgets`?
- [ ] ¿Reemplacé `atoi`/`atof` por `strtol`/`strtod` con validación de `endptr`
  y `errno`?
