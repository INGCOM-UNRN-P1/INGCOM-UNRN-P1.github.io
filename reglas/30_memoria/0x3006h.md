...
el contrato es evidente en el módulo, pero la anotación de propiedad no se
omite.
...
- **Propiedad compartida:** si el recurso se comparte (por ejemplo, un buffer
  global del módulo), documentalo y liberalo una sola vez, al final del
  programa.
- **Transferencia parcial:** una función puede devolver un puntero al interior
  de
...
* {ref}`0x3009h` — los retornos `NULL` deben documentarse junto con la
  propiedad.
* {ref}`0x300Eh` — el comportamiento frente a argumentos nulos se documenta con
  el contrato.
* {ref}`0x301Dh` — en TAD opacos, la propiedad recae siempre en el módulo.
