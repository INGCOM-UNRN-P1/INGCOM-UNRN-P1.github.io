...
punteros (un `size_t` no admite `NULL`). La decisión se escribe como
precondición
...
valida su entrada. El contrato es una incógnita que sólo se descubre al
ejecutar.
...
- **Doble puntero:** documentá también si `*ptr` puede ser `NULL`, no sólo
  `ptr`.
...
* {ref}`0x2003h` — la documentación estructurada aloja precondiciones y
  postcondiciones.
