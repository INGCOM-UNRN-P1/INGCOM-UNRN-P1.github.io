...
C11 §7.19 define `size_t` en `<stddef.h>` como el tipo sin signo del resultado
de
...
- **`printf`:** usá `%zu` para `size_t`; con `%d` el comportamiento es
  indefinido
...
* {ref}`0x100Dh` — la condición simple del lazo se apoya en cotas del tipo
  correcto.
