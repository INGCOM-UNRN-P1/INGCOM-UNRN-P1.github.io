...
puntero, que es exactamente lo que se guarda en el arreglo. Es el uso legítimo
de
...
- **Desbordamiento del producto:** aun con el `sizeof` correcto, `n *
  sizeof(*p)`
...
* {ref}`0x300Bh` — regla general: siempre `sizeof`, preferentemente
  `sizeof(*ptr)`.
...
- `malloc(): corrupted top size` o `free(): invalid next size (fast)` en
  `glibc`.
