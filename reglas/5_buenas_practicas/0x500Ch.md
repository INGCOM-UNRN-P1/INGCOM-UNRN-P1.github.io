...
* {ref}`0x0011h` — el módulo expone su interfaz por el `.h`, no copiando el `.c`.
* {ref}`0x5013h` — las declaraciones compartidas se formalizan en la cabecera.
:::
...
- **Extensión engañosa:** un archivo `.h` que en realidad contiene definiciones
  también viola el espíritu de la regla; la interfaz va en `.h` y el cuerpo en `.c`.
...
- [ ] Si uso X-macros, ¿el archivo incluido no se compila solo y usa otra extensión?

## Reglas relacionadas

* {ref}`0x0011h` — la interfaz se expone por la cabecera, no copiando el fuente.
* {ref}`0x5013h` — lo compartido se declara en el `.h`, no con `extern` en el `.c`.
* {ref}`0x5005h` — la compilación separada presupone archivos con secciones claras.
