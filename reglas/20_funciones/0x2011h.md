---
title: "Regla 0x2011h: Prohibición de asignaciones múltiples a una variable sin lectura intermedia (dead store)"
short_title: "0x2011h"
...
{ref}`0x2006h`, la presencia de una variable con varias escrituras y pocas
...
({ref}`0x0001h`) y porque suele delatar errores de lógica. La contracara es
{ref}`0x7001h`: si una variable debe evolucionar, cada paso debe leerse o
justificarse.
...
- **Aritmética con signo:** un dead store puede esconder una operación con
  overflow, que sí es UB. El caso de {ref}`0x2018h` muestra cómo una
...
- **Variables de depuración:** si una variable solo existe para inspección con
  `gdb`, se elimina antes de la entrega ({ref}`0x0202h`).
...
* {ref}`0x7001h` — inicializar a un valor conocido; no a un valor descartado.
* {ref}`0x2006h` — el alcance mínimo reduce las variables longevas y los dead stores.
* {ref}`0x0202h` — el código residual se elimina, no se comenta.
