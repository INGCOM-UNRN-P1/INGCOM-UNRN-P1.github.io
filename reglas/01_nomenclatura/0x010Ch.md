---
title: "Regla 0x010Ch: Auditor de identificadores reservados con doble guion bajo o guion bajo inicial"
short_title: "0x010Ch"
...
:class: note
Código de regla: `0x010Ch` | Soporta corrección automática: **No**
...
regla coincide con {ref}`0x010Ah`; se mantienen ambas por trazabilidad histórica
del catálogo.
...
`static` restringe el enlace del global conforme a {ref}`0x0106h`.
...
- [ ] Uso `static` o `g_` para los globales ({ref}`0x0106h`).
- [ ] Distingo un `_` inicial de archivo de un `_` local y lo evito igual.

## Reglas relacionadas

* {ref}`0x010Ah` — misma prohibición de prefijos reservados.
* {ref}`0x5003h` — guardas de inclusión con nombres propios.
* {ref}`0x0106h` — restringir el alcance de las globales.
* {ref}`0x3004h` — sufijo `_t` reservado para tipos.
* {ref}`0x0108h` — identificadores restringidos al conjunto ASCII.
